...
└── internal/                            Go บังคับเองว่า module ภายนอก import ไม่ได้
    ...
    └── domain/                          ── ส่วนที่มีตรรกะเฉพาะ
        ...
        ├── user/
        │   ├── handler.go               สร้างผู้ใช้ — bcrypt + ผูกบทบาทจาก tb_role
        │   └── handler_test.go
        ├── stock/
...
**ไม่ใช่ตัวบังคับสิทธิ์** ตัวที่บังคับคือ `authz.GuardResource` บนเส้นทางแต่ละเส้น
ซึ่งอ่าน View เดียวกันนี้ คำตอบของ endpoint นี้จึงตรงกับสิ่งที่เส้นทางจะทำเสมอ
ไม่ใช่สำเนาที่ค่อย ๆ เพี้ยน

`vw_user_privilege` รวมสิทธิ์ข้ามบทบาทแบบ union ให้แล้ว ผู้ใช้ถือได้หลายบทบาท
บทบาทใดให้ผ่านก็ผ่าน

`POST /users` ผูกบทบาทให้ในทรานแซกชันเดียวกับที่สร้างบัญชี โดยหาแถวที่ `role_code`
ตรงกับ `user_level` ที่เพิ่งเขียนลงไป ไม่เจอบทบาท = ล้มทั้งทรานแซกชัน ไม่ใช่สร้างผู้ใช้
ทิ้งไว้แบบไม่มีสิทธิ์ ผู้ใช้ทุกคนจึงมีแถวใน `tb_user_role` เสมอ และรายการสิทธิ์ที่ว่าง
แปลว่า "ไม่มีสิทธิ์จริง ๆ" ไม่ใช่ "ยังไม่ได้ผูกบทบาท"

บัญชีที่สร้างก่อน 13 ก.ย. 2026 ไม่มีบทบาท — ดูคำสั่ง backfill ใน
`PenbunSQL/README.md` §Migration Note
...
### `POST /users` — ระดับสิทธิ์มาจาก `tb_role`

รายการ `user_level` ที่รับได้ไม่ได้เขียนตายไว้ใน Go แล้ว `assignRole` หาแถวใน `tb_role`
ที่ `role_code` ตรงกับค่าที่ส่งมา และ `is_active = 1` ไม่เจอคือ **400** พร้อมฟิลด์
`user_level` ไม่ใช่ 500 เพราะเมื่อฐานเป็น v12 ขึ้นไป "ไม่มีบทบาทนี้" แปลว่าค่าที่ส่งมาผิด

เพิ่มบทบาทในฐานแล้ว API รับค่านั้นทันทีโดยไม่ต้อง deploy ใหม่ — PenbunSQL v13 เพิ่ม
`WAREHOUSE` · `DELIVERY` · `VIEWER` ด้วยวิธีนี้

การผูกบทบาทอยู่ในทรานแซกชันเดียวกับการสร้างบัญชี ล้มก็ล้มทั้งคู่ ไม่มีบัญชีที่สร้าง
สำเร็จแล้วไม่มีบทบาทค้างอยู่ในฐาน

### `PUT /auth/me`

ให้เจ้าของบัญชีแก้ `full_name` และ `email` ของตัวเอง ขอบเขตคือแถวของผู้เรียกเสมอ —
`user_id` มาจาก token ไม่ได้มาจาก path หรือ body จึงไม่ต้องมีตัวกรองสิทธิ์

`user_name` และ `user_level` ไม่มีทางเดินผ่านปลายทางนี้เลย ไม่ใช่แค่ไม่ได้เขียนไว้ในเอกสาร
ชื่อผู้ใช้ถูกเขียนไว้ใน `update_by` ของทุกแถวในระบบ เปลี่ยนแล้วบันทึกย้อนหลังกำพร้า
ส่วนระดับสิทธิ์คือสิ่งที่คนแก้ได้จะยกให้ตัวเองได้ การแก้ผู้ใช้คนอื่นเป็น `PUT /users/{id}`
ซึ่งเป็นงานของ ADMIN และยังไม่มี

ช่องว่างถูกเก็บเป็น `NULL` ไม่ใช่สตริงว่าง (`NULLIF`) และคำตอบคือโปรไฟล์ที่อ่านกลับมา
จากฐาน ไม่ใช่ค่าที่รับเข้ามา หน้าจอจึงไม่โชว์ค่าที่ไม่ได้อยู่ในฐานจริง

ความยาวตรวจให้ตรงกับคอลัมน์ — `full_name` 150 · `email` 100 กว้างกว่านี้ validation จะ
ปล่อยผ่านแล้วไปตายตอน `UPDATE` กลายเป็น 500 ทั้งที่เป็นคำขอที่ผิด ต้องเป็น 400
...
| กลุ่ม | เส้นทาง | จำนวน |
| :--- | :--- | ---: |
| การเข้าสู่ระบบ | `/auth/login` `/refresh` `GET /me` `PUT /me` `/change-password` `/logout` · `/users/{id}/unlock` | 7 |
